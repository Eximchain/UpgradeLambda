...
const (
	...
	CNodeType              = "node_type"
	CNotifyBucket          = "notify-bucket"
	CUpgradeARN            = "upgrade-arn"
...
func SNSHandler(ctx context.Context, snsEvent events.SNSEvent) {
	...
	for _, record := range snsEvent.Records {
		...
		fmt.Printf("1 Oct 2018 1119 [%s %s] Message = %s \n", record.EventSource, snsRecord.Timestamp, snsRecord.Message)
		...
		if err := json.Unmarshal([]byte(snsRecord.Message), &msg); err == nil {
			...
			if err3 := json.Unmarshal([]byte(msg.NotificationMetadata), &notificationInfo); err3 == nil {
				...
				upgradeBucket = notificationInfo[CBucketName]
				notifyBucket = notificationInfo[CNotifyBucket]
				nodeType = notificationInfo[CNodeType]
				upgradeARN = notificationInfo[CUpgradeARN]
				...
				getS3BucketObj := func(downloader *s3manager.Downloader, bucketName, bucketKey string) (result []byte, err error) {
					...
					if n, err = downloader.Download(buf, &s3.GetObjectInput{
						...
						fmt.Printf("Bucket: %s, Key: %s, err: %v\n", bucketName, bucketKey, err)
					}
					return
				}
					...
					for _, inst := range resp.Reservations[idx].Instances {
						...
						if InstanceID != "" && DNSName != "" {
							...
							if upgradeScriptBytes != nil {
								...
								fmt.Printf("Copying bytes to %s on %s\n", destName, DNSName)
								if err := sshconfig.CopyBytesToFileAndVerify(upgradeScriptBytes, destName, "0755"); err != nil {
									...
									var args string
									if upgradeBucket != "" {
										args = fmt.Sprintf("%s %s %s %s %s", notifyBucket, nodeType, InstanceID, upgradeBucket, upgradeARN)
									}
									progargs := fmt.Sprintf("%s", destName)
									if args != "" {
										progargs = fmt.Sprintf("%s %s", progargs, args)
									}
									// Note screen is sensitive to placement of the quotes (") symbol
									cmd := fmt.Sprintf(`screen -d -m %s`, progargs)
