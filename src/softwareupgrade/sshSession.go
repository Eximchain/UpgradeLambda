...
func (sshConfig *SSHConfig) MakeSudo() (result *SSHConfig) {
	sshConfig.sudo = "sudo "
	return sshConfig
}

// ClearSudo removes Sudo from Copy, CreateDirectory, ChangeFileOwnership and Run
func (sshConfig *SSHConfig) ClearSudo() (result *SSHConfig) {
	sshConfig.sudo = ""
	return sshConfig
}

// EnableVerbose enables verbose response for file transfers
func (sshConfig *SSHConfig) EnableVerbose() (result *SSHConfig) {
	sshConfig.verbose = "v"
	return sshConfig
}

// DisableVerbose disables verbose response for file transfers
func (sshConfig *SSHConfig) DisableVerbose() (result *SSHConfig) {
	sshConfig.verbose = ""
	return sshConfig
}
...
// NewSSHConfigKeyBytes creates a new SSHConfig based on the given information
...
// CopyRemoteFileToLocalFile copies the given remote file to the local file, setting the permissions for the local file.
func (sshConfig *SSHConfig) CopyRemoteFileToLocalFile(remoteFilename, localFilename, permissions string) (err error) {
	if sshConfig.session == nil {
		if !sshConfig.autoOpenSession {
			panic("No SSH session opened.")
		}
	...
	f, err1 := os.Create(localFilename)
	if err1 != nil {
		err = err1
		return
	}
	...
	go func() {
		defer wg.Done()
		w, err1 := sshConfig.session.StdinPipe()
		if err1 != nil {
			err = err1
			return
		}
		defer w.Close()
		r, err2 := sshConfig.session.StdoutPipe()
		if err2 != nil {
			err = err2
			return
		}
		var (
			permissions, filename string
			len                   int
		)
		fmt.Fprint(w, "\x00") // Tells remote to send file info
		fmt.Fscanln(r, &permissions, &len, &filename)
		fmt.Printf("permissions: %s, len: %d, filename: %s\n", permissions, len, filename)
		fmt.Fprint(w, "\x00") // Tells remote to send file data
		writer := bufio.NewWriterSize(f, len)
		if writtenCount, err3 := io.CopyN(writer, r, int64(len)); err3 != nil {
			err = err3
		} else {
			writer.Flush()
			fmt.Fprint(w, "\x00") // Tells remote to close
			if writtenCount != int64(len) {
				err = errors.New("Copied size not equal filesize")
			}
		}
	}()
	cmd := fmt.Sprintf("%s/usr/bin/scp -%sf %s", sshConfig.sudo, sshConfig.verbose, remoteFilename)
	err = sshConfig.session.Run(cmd)
	wg.Wait() // waits for the coroutine to complete
	return
}
...
// rawRun runs a command on the given SSH environment, usage: output, err := Run("ls")
